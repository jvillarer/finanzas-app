...
const TABS = [
  ...
  },
  {
    ...
    label: "Lani",
    icon: (active: boolean) => (
      <span className="text-xl leading-none" style={{ filter: active ? "none" : "grayscale(1) brightness(0.4)" }}>🐑</span>
    ),
  },
  {
    href: "/estadisticas",
    ...
    >
      {TABS.map(({ href, label, icon }) => {
        ...
        return (
          ...
          >
            {icon(activo)}
            <span className="text-[10px] font-bold tracking-wide" style={{ color: activo ? "#ffffff" : "#4b5563" }}>
              {label}
            </span>
          </Link>
        );
