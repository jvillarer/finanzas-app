...
function GraficaHero({ meses }: { meses: DatoMes[] }) {
  ...
  const W=350, H=120, P={l:8,r:8,t:10,b:18};
  ...
  // Etiqueta del máximo en el eje Y
  const maxLabel = fmt(maxV/1.08);
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" preserveAspectRatio="none" style={{display:"block"}}>
      ...
      {/* Referencia Y — máximo */}
      <text x={P.l} y={P.t-2} fontSize="8" fill="rgba(255,255,255,0.35)" fontFamily="Inter,sans-serif">{maxLabel}</text>
      {/* Gastos */}
      <path d={area(xs,yOut)} fill="url(#outGrad)"/>
      <path d={curva(xs,yOut)} fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="3 4"/>
      {/* Ingresos */}
      ...
      {meses.map((m,i)=>(
        <text key={i} x={xs[i]} y={H-3}
          ...
          fill={i===meses.length-1?"#7dd3a8":"rgba(255,255,255,0.45)"}
    ...
    <div style={{position:"relative",width:tamaño,height:tamaño,flexShrink:0}}>
      <svg width={tamaño} height={tamaño}>
        ...
        {datos.map((d,i)=>{
          ...
          const grados  = pct*360-gapGrados;
          const dashLen = (grados/360)*circ;
          const rot     = rotActual+gapGrados/2;
          rotActual    += pct*360;
...
function BarrasDiaSemana({ gastosPorDia }: { gastosPorDia: number[] }) {
  ...
  const maxIdx = gastosPorDia.indexOf(max);
  return (
      ...
      {gastosPorDia.map((v,i)=>{
        ...
        return (
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{
              width:"100%",
              height:`${Math.max(pct*56,v>0?4:2)}px`,
              borderRadius:"4px 4px 0 0",
              background: esMax ? ACCENT_DK : v>0 ? ACCENT+"99" : HAIR,
              transition:"height 0.4s cubic-bezier(0.34,1.56,0.64,1)",
            }}/>
            <span style={{
              fontSize:9, fontWeight: esMax?800:600,
              color: esMax ? INK : MUTED,
              letterSpacing:"0.2px",
            }}>{DIAS_SEMANA[i]}</span>
          </div>
        );
...
export default function EstadisticasPage() {
  ...
  // Gráfica hero (siempre 6 meses hacia atrás)
  const datosMeses: DatoMes[] = useMemo(()=>{
    const n=6, ahora=hoy();
    return Array.from({length:n},(_,i)=>{
      const d  = new Date(ahora.getFullYear(), ahora.getMonth()-(n-1-i),1);
      const fn = new Date(d.getFullYear(), d.getMonth()+1,0);
      const txs= transacciones.filter(t=>t.fecha>=fechaStr(d)&&t.fecha<=fechaStr(fn));
      return {
        m:       mesLabel(d),
  ...
  // Gastos por día de la semana (del periodo seleccionado)
  const gastosPorDiaSemana: number[] = useMemo(()=>{
    const totales = [0,0,0,0,0,0,0];
    enPeriodo.filter(t=>t.tipo==="gasto").forEach(t=>{
      const d = new Date(t.fecha+"T12:00:00");
      totales[d.getDay()] += Number(t.monto);
    });
    return totales;
  },[enPeriodo]);
  ...
  return (
    <div style={{width:"100%",height:"100dvh",background:PAPER,display:"flex",flexDirection:"column",overflow:"hidden",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display','Inter',sans-serif"}}>
      ...
      {/* ══ HEADER OSCURO ══ */}
      <div style={{
        flexShrink:0, background:INK, color:"#fff",
        paddingTop:"calc(env(safe-area-inset-top) + 16px)",
        paddingLeft:22, paddingRight:22, paddingBottom:16,
        position:"relative",
      }}>
        {/* Textura */}
        ...
        <div style={{position:"relative"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
            ...
            <p style={{fontSize:10.5,color:"rgba(255,255,255,0.55)",fontWeight:700,letterSpacing:"1.4px",textTransform:"uppercase"}}>
              Análisis · {labelPeriodo}
            </p>
          </div>
          <h1 style={{fontSize:32,color:"#fff",fontWeight:800,letterSpacing:"-0.8px",lineHeight:1,fontStyle:"italic"}}>
        ...
        <div style={{marginTop:16,position:"relative"}}>
          <p style={{fontSize:10,color:"rgba(255,255,255,0.45)",fontWeight:700,letterSpacing:"1.2px",textTransform:"uppercase",marginBottom:4}}>Balance neto</p>
          <div style={{display:"flex",alignItems:"baseline",gap:10,flexWrap:"wrap"}}>
            <h2 style={{fontSize:48,fontWeight:800,color:balance>=0?ACCENT:RUST,letterSpacing:"-1.8px",lineHeight:0.95,fontStyle:"italic"}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(125,211,168,0.12)",borderRadius:8,padding:"5px 10px"}}>
              ...
              <span style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:500}}>ingresos</span>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(200,80,62,0.12)",borderRadius:8,padding:"5px 10px"}}>
              ...
              <span style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:500}}>gastos</span>
            </div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:14,marginTop:4,marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            ...
            <span style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontWeight:600}}>Ingresos</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:16,height:2,background:"rgba(255,255,255,0.4)",borderRadius:1,borderTop:"1px dashed rgba(255,255,255,0.4)"}}/>
            <span style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontWeight:600}}>Gastos</span>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:96} as React.CSSProperties}>
        ...
        {/* KPI CARDS — sin spark bars, con contexto útil */}
        <div style={{padding:"18px 20px 0",display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div style={{background:CARD,borderRadius:18,padding:16,border:`1px solid ${HAIR}`,boxShadow:"0 1px 3px rgba(15,47,47,0.05)"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:10.5,color:MUTED,fontWeight:600}}>
                {tasaAhorro>=20?"¡Excelente!":tasaAhorro>=10?"Bien":"Mejorable"}
              </span>
              {periodo!=="Todo" && difTasa!==0 && (
                <span style={{fontSize:10,fontWeight:700,color:difTasa>0?ACCENT_DK:RUST}}>
                  {difTasa>0?"+":""}{difTasa}pp vs ant.
                </span>
              )}
            </div>
          </div>
          ...
          <div style={{background:CARD,borderRadius:18,padding:16,border:`1px solid ${HAIR}`,boxShadow:"0 1px 3px rgba(15,47,47,0.05)"}}>
            ...
            <p style={{fontSize:34,fontWeight:800,color:INK,letterSpacing:"-1px",lineHeight:1,fontStyle:"italic",marginBottom:8}}>
              {fmt(gastoDiario)}
            </p>
            {/* Barra de contexto vs ingreso diario */}
            {ingresos>0 && (
              <>
                <div style={{height:4,background:HAIR,borderRadius:4,overflow:"hidden",marginBottom:8}}>
                  <div style={{
                    height:"100%",
                    width:`${Math.min((gastoDiario/(ingresos/diasPeriodo))*100,100)}%`,
                    background:gastoDiario>(ingresos/diasPeriodo)?RUST:ACCENT_DK,
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  {periodo!=="Todo" && pctDiario!==0 && (
                    <span style={{fontSize:10,fontWeight:700,color:pctDiario<0?ACCENT_DK:RUST}}>
                      {pctDiario>0?"+":""}{pctDiario}% vs ant.
                    </span>
                  )}
                </div>
              </>
                ...
                <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
                  {categorias.slice(0,4).map(c=>(
                    <div key={c.nombre} style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{width:3,height:"100%",minHeight:18,borderRadius:2,background:c.color,flexShrink:0}}/>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                          <p style={{fontSize:12,color:INK,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:90}}>{c.nombre}</p>
        ...
        {/* CUÁNDO GASTAS — barras por día de la semana */}
            ...
            <div style={{background:CARD,borderRadius:22,padding:18,border:`1px solid ${HAIR}`,boxShadow:"0 1px 3px rgba(15,47,47,0.05)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:16}}>
                <div>
                  ...
                  <p style={{fontSize:11,color:MUTED,marginTop:2}}>Total de gastos acumulados</p>
                </div>
                <span style={{fontSize:11,fontWeight:700,color:ACCENT_DK}}>
                  {DIAS_SEMANA[gastosPorDiaSemana.indexOf(Math.max(...gastosPorDiaSemana))]}
                </span>
              </div>
              <BarrasDiaSemana gastosPorDia={gastosPorDiaSemana}/>
              <div style={{marginTop:10,display:"flex",justifyContent:"space-between",fontSize:10,color:MUTED,fontWeight:600}}>
              ...
              {top5.map((t,idx)=>{
                ...
                return (
                  <div key={t.id} style={{
                    ...
                    display:"grid", gridTemplateColumns:"26px 38px 1fr auto", gap:12, alignItems:"center",
                  }}>
                    <span style={{fontSize:13,fontWeight:700,color:MUTED,fontStyle:"italic"}}>{num}</span>
                    ...
                    <div style={{minWidth:0}}>
                      <p style={{fontSize:13,fontWeight:700,color:INK,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{desc}</p>
        ...
        {enPeriodo.length===0 && (
          <div style={{padding:"60px 32px",textAlign:"center"}}>
            ...
            <p style={{fontSize:14,color:MUTED,lineHeight:1.5}}>Agrega transacciones para ver tus estadísticas aquí</p>
          </div>
        )}
